...
def getBoundingCenters(xmin, xmax, ymin, ymax, centers_x, centers_y, centers_xR, centers_yR, Y_input, labeles, label):
...
theta_degrees = 75      # Angle in degrees
...
L_input = 140 * 5.1385 / mt.sin(theta_radians)   # Camera height in PIXELS
...
while True:
    ...
    for i in range(len(detections)):
        ...
        print(f"{classname} Panjang: {(xmax - xmin)}, Lebar: {ymax - ymin}, Pixel")
        ...
        if conf > min_thresh:
            ...
            getBoundingCenters(xmin, xmax, ymin, ymax, centers_x, centers_y, centers_xR, centers_yR, Y_input, labeles, labelname)
    ...
    if key == ord('q') or key == ord('Q'):
        ...
        cv2.waitKey()
    elif key == ord('o') or key == ord('O'):  # Changed from 'p'
